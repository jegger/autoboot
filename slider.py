...
class RTBoundarySlider(Widget):
    def __init__(self, **kwargs):
        ...
        self.min=Label(pos=(self.last_size[0]+self.delta_x_size+self.x-30,self.y-102), text="0", color=(0,0,0,1), bold=True)
        self.add_widget(self.min)
        self.max=Label(pos=(self.last_pos[0]-70,self.y-102), text="0", color=(0,0,0,1), bold=True)
        ...
        self.num_0 = Label(pos=(self.x-49, self.y-79), text="00:00", color=(0,0,0,1))
        self.add_widget(self.num_0)
        self.num_12 = Label(pos=(self.x+(self.width/2)-49, self.y-79), text="12:00", color=(0,0,0,1))
        self.add_widget(self.num_12)
        self.num_24 = Label(pos=(self.x+self.width-49, self.y-79), text="24:00", color=(0,0,0,1))
    ...
    def set_value(self):
        ...
        if self.max_time-self.min_time < 120:
            ...
            self.graphic.canvas.add(Rectangle(size=(self.width+80, self.height+90), pos=(self.x-40, self.y-67)))
        else:
            self.stay_off = False
    ...
    def draw_unchangable(self):
        ...
        self.g_values.canvas.add(Rectangle(size=(1,40), pos=(self.x, self.y-20)))
        self.g_values.canvas.add(Rectangle(size=(1,40), pos=(self.x+(self.width/2), self.y-20)))
        self.g_values.canvas.add(Rectangle(size=(1,40), pos=(self.x+self.width, self.y-20)))
        
        self.g_values.canvas.add(Color(0, 0, 0, 1))
        self.g_values.canvas.add(Rectangle(size=(self.width+self._border*2, self.height+self._border*2), pos=(self.x-self._border, self.y-self._border)))
    ...
    def draw_right(self):
        ...
        self.slider_butR.pos=((self.last_size[0]+self.delta_x_size+self.x)-self.slider_butR.width/2, self.y-40)
        self.slider_butL.pos=((self.last_pos[0]-self.slider_butL.width/2, self.y-40))
    ...
    def on_touch_up(self, touch, *kwargs):
        if touch.grab_current == self:
            RTDatabase_boot.database.insert_day(day=self._day, start_hour=self.value_h_min, start_min=self.value_m_min, stop_hour=self.value_h_max, stop_min=self.value_m_max, stay_off=self.stay_off)
